...
def get_product_variations(product_id, consumer_key, consumer_secret,
                           woo_commerce_url):
    wpapi = setup_wp_api(consumer_key, consumer_secret, woo_commerce_url)
...
def get_category_name(category_id, consumer_key, consumer_secret,
                      woo_commerce_url):
    wpapi = setup_wp_api(consumer_key, consumer_secret, woo_commerce_url)
...
def get_from_category(category_id, restaurant, consumer_key, consumer_secret,
                      woo_commerce_url):
    wpapi = setup_wp_api(consumer_key, consumer_secret, woo_commerce_url)
    ...
    category_name = get_category_name(category_id, consumer_key,
                                      consumer_secret, woo_commerce_url)
    ...
    for product in products.json():
        ...
        if product['type'] == 'variable':
            variations = get_product_variations(product['id'], consumer_key,
                                                consumer_secret,
                                                woo_commerce_url)
